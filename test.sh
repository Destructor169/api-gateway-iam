...
curl -s -i -H "Authorization: Bearer $TOKEN" "$GATEWAY_URL/api/data"
echo -e "\n"

echo "5. Testing Rate Limiting (Sending 10 requests quickly)..."
for i in {1..10}
do
   echo -n "Request $i: "
   curl -s -o /dev/null -w "%{http_code}\n" -H "Authorization: Bearer $TOKEN" "$GATEWAY_URL/api/data"
done
